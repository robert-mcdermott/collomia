...
The current runtime event model defines these kinds. One-shot runs emit the
applicable public subset; rows below identify internal or TUI-only events:
...
TUI-only `/orchestrate` workflow and internal evaluations; there is no
