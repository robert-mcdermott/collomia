...
| permissions | OS sandbox | experimental | built-in backends: macOS Seatbelt, Linux Landlock, Windows 11 AppContainer + Job Object; active platform: sandbox-exec (Seatbelt); workspace write confinement; broad reads; full network denial; localhost remains available when remote network is denied; process-group termination is best effort; enable with permissions.sandbox=auto|require |
