...
| Work-mode non-Git artifact delivery | Work prompt, structured plan, `write_file`, `validate_artifact`, completion controller, typed events | An ordinary non-Git folder produces a Markdown report, accepts one path- and digest-bound validation after the final write, emits the typed receipt and file-change manifest, and completes without a Developer test loop or controller retry; when two artifacts change and only one is validated, the first completion notice names only the remaining workspace-relative path, omits the accepted artifact, preserves unknown-path mutations separately, and completes after one targeted validation. |
...
| Structured failed-tool recovery and completion reporting | Completion controller ledger, tool-call receipts, external-data provenance framing, `update_plan.resolved_failures`, system prompt, plan assessment, retained final answer | Transcript-shaped failed `edit_file`/successful `run_command` and failed `web_search`/successful `web_fetch` paths complete only when the plan binds exact provider-envelope call IDs; the notice advertises bounded valid receipt candidates and distinguishes them from IDs embedded in output; a mistaken external-data marker points to the real successful call without being accepted as proof; unrelated same-risk tools and unobserved recovery IDs cannot clear a failure; changed invalid guesses do not renew the two-intervention bound; skipped and blocked dispositions require matching terminal steps; metadata-only repair returns the original answer without another provider request. |
| Standard completion diagnostics and progress lease | Standard completion controller, verification classifier, tool-result receipts, goal outcome mapping, provider-iteration budget | A transcript-shaped static-site run that passes an inline smoke check receives the exact refusal reason and `verification_note` route on the next provider request and closes after one correction; a recognized direct verifier returns a tracked-state receipt; a verification-only terminal gap is `needs_verification`, not `blocked`; novel evidence may cross the old whole-turn limit, identical evidence exhausts the no-progress lease, and continuous writes stop at the hard envelope. |
| Skill bundle reads | Path guard read roots, built-in read/list/search tools, mutation path, skill catalog | An active skill's directory is readable without `allow_outside_workspace`; the same directory refuses write_file and edit_file and leaves its files unchanged; a symlink inside a bundle cannot resolve out of it; disabled skills and untrusted project skills are excluded from the readable set. |
| Node verification and unrecognized-check feedback | Verification recognizer, project detection, graph completion gap, tool-result notice, stalled-node blocker | `node --test` and a script in a conventional test location are accepted while `node index.js` and `node -e` are refused; a node in a manifest-less project completes on the focused smoke test the proposal contract told it to create; a passing command the table does not cover is named in the tool result while the remediation lease still runs and named again in the blocker, with the project's detected verifiers when it has any; ordinary read commands receive no such notice. |
| Finished-goal release | Runtime release/archive path, terminal outcome classification, TUI `/orchestrate done`, ordinary-prompt release, terminal guard messages | A graph that has stopped returns the session to Standard mode by command or by the next ordinary prompt without deleting its transcript or evidence; release refuses a running graph and names pause and cancel, refuses an unapproved proposal, and refuses `awaiting_review`/`awaiting_verification` while naming integrate, verify, and waive; a blocked graph keeps its retry path through a prompt; the unreconciled-worktree refusal is unchanged; and every terminal guard names the commands that apply to its own outcome. |
