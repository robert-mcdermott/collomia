...
| Skipped-versus-blocked completion reporting | Completion controller notice, system prompt, plan completion assessment | The notice names `skipped` for an action that proved unnecessary or was achieved another way and `blocked` only for work that cannot be completed, and states that blocked ends the turn; a skipped step with a reason leaves the plan ready while a blocked one does not. |
