...
| Structured failed-tool recovery and completion reporting | Completion controller ledger, tool-call receipts, `update_plan.resolved_failures`, system prompt, plan assessment, retained final answer | A transcript-shaped failed `edit_file` followed by a successful `run_command` completes only when the plan binds both exact call IDs; unrelated same-risk tools and unobserved recovery IDs cannot clear the failure; skipped and blocked dispositions require matching terminal steps; metadata-only repair returns the original answer without another provider request. |
