...
- **Planning and agent workflow**
  ...
  - The experimental Orchestrated Goal graph is Collomia's evidence-gated durable execution mode: the model proposes the work, while the runtime owns readiness, attempts, evidence freshness, recovery treatment, terminal state, aggregate accounting, and aggregate bounds. `/orchestrate <goal>` creates a fresh read-only, acceptance-bearing proposal; `/orchestrate approve` activates it once; and status/cooperative-pause/explicit-resume/safe-retry/whole-graph-cancel expose durable node, attempt, failure, evidence, accounting, and budget state. A terminal graph yields automatically to another `/orchestrate <goal>` in the same session through an append-only tombstone; explicit terminal cancel performs the same archive action without deleting transcript or evidence. Proposal statuses/evidence never become runtime completion: approval starts every node pending, and `/plan off` safely cancels an unapproved proposal before restoring execution mode. Status separates proposal-plus-primary, automatic-read, and automatic-writer work, including every completed provider-call iteration (also failures and retries), input/output tokens, honestly available estimated cost, elapsed time, and active time. A whole-graph envelope defaults to 96 provider iterations, 1,000,000 tokens, $5 estimated cost when pricing is complete, and 30 minutes active after approval, and `options.orchestration_*` sets each; only implausible values are refused, repository content cannot widen it, and restored graphs retain the envelope they were created with. The token envelope charges prompt tokens the provider had to read anew plus everything generated, so a long node's re-sent context does not consume the ceiling; cached tokens are reported but not charged. Every bound is a speed bump rather than a wall: reaching one keeps the accepted nodes and retained candidates and asks the user, and `/orchestrate extend` grants another envelope of the configured size as often as they decide to, with unfinished nodes resuming in new attempts rather than replaying interrupted ones. Inside that envelope, `max_iterations` is a consecutive primary no-progress lease: novel durable successful tool evidence or a resolved recoverable failure renews it, equivalent repetition does not. Once the runtime records an exact completion gap, a four-cycle remediation lease renews for a real workspace repair, a novel machine-observed verification failure, or evidence that closes the gate; identical failures, different command wrappers, and unrelated output cannot prolong it. Conservative write-ahead effect history remains separate from observed workspace freshness, so a completed process or smoke check preserves earlier verification when the Git state token is unchanged. Proposal guidance prefers one to three coherent outcome nodes for scoped changes and four to six only for broad goals, treats twelve as a maximum, defaults end-to-end changes to primary, and requires the first mutating node in a project without an applicable test surface to create a focused smoke test. Proposal history is compacted once before node execution, and active context is compacted again when it would consume at least one-eighth of the remaining graph allowance and has grown materially since the last compaction, so a context already reduced to a summary is not repeatedly re-summarized; those summary requests remain fully accounted. A passing verifier instructs the model to stop at the current node, and each accepted non-final node creates a zero-provider runtime handoff so the next request does not resend the previous node's tool loop; the full transcript remains durable. At most two independently ready approved `read_only` nodes can use governed automatic readers. An explicitly candidate-only graph may run one bounded wave of at most two ready pairwise-disjoint terminal `isolated_write` nodes from a common clean Git commit; approval preflights that base and names dirty paths without consuming the proposal. Candidate graphs cannot contain primary nodes or depend on retained writers. Dispatch and every detected child verification command keep their ordinary permission/hook decisions, candidates stay in retained worktrees, and passing scope/base/freshness evidence stops the graph for review without touching the parent workspace or unlocking dependents. Controlled comparisons retain read fan-out for substantive independent reads while making its extra model work visible; trivial primary work and dependency-serial reads remain serial. After a parent mutation, recognized verification—including an exact redundant workspace `cd ... &&` and final `2>&1` wrapper—must pass against the current Git workspace state and returns a positive state-bound receipt; masking shell composition remains rejected. Interrupted safe reads may be recomputed in a fresh attempt, but ambiguous parent mutations and interrupted isolated writers are blocked rather than replayed; manual retry rejects that same ambiguity. Every worktree the runtime creates is recorded against its node and attempt at creation, and `/orchestrate reconcile` observes what is actually left in each one — present, empty, missing, no longer registered with Git, or unmoored from its base commit — so a remembered path is never reported as though it were current. `/orchestrate discard <node-id>` removes a tree you no longer want; it refuses one nobody has reconciled, requires `confirm` when the tree still holds changes, and declines a directory Git no longer registers. Neither command is reachable by the model or by an autonomy mode, and archiving a terminal graph waits until its worktrees have been observed. A restart reproduces a multi-worker schedule exactly: the same nodes are reselected in the same order, no interrupted attempt is resumed in place, and the spent read envelope carries across so the restart is charged for the starts it re-spends. A publication into your workspace that never recorded an outcome blocks integration, combined verification, and waiver until you resolve it — `/restore integration <id>` puts the prior bytes back and `/restore integration <id> keep` records that you are keeping what was published — because each of those steps is a claim about a combined workspace that may hold some of a candidate's files and not others. There is no configuration/headless opt-in, automatic candidate selection/integration/reuse, or optional-branch/node cancellation; Standard mode remains the default.
