...
- **Planning and agent workflow**
  ...
  - The experimental OG-1/OG-2A primary graph is available through an explicit TUI-only flow: `/orchestrate <goal>` creates a fresh read-only, acceptance-bearing proposal; `/orchestrate approve` activates it once; and status/cancel/resume expose durable node, attempt, failure, and evidence state. It remains one serial primary lane with ordinary permissions and verification—no automatic agents, configuration switch, or headless opt-in—and Standard mode remains the default.
