...
func TestWriteGlobalStarterIncludesProvidersPermissionsAndOptions(t *testing.T) {
	...
	if len(permissions) != 5 || string(permissions["mode"]) != `"ask"` || string(permissions["allow_outside_workspace"]) != "false" || string(permissions["sandbox"]) != `"auto"` || string(permissions["sandbox_allow_network"]) != "true" || string(permissions["sandbox_allow_read_outside_workspace"]) != "true" {
