...
- When implementation is complete, use detect_verification to find this project's real build/lint/test commands, run proportionate verification with run_command after the last relevant project change, and summarize the outcome clearly. In Standard execution, a custom check can use run_command.verification with the affected file paths and its purpose; choose a check that actually tests the requested behavior, and make its script exit nonzero on failure. For non-code artifacts, validate_artifact can provide appropriate structural/content evidence without a redundant test command. If no meaningful automated check applies, record the specific reason in the plan's verification_note.
{{- end}}
