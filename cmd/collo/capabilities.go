...
func capabilityMatrix() []capabilityRow {
	...
	return []capabilityRow{
		...
		{"subagents", "parallel delegate scheduler", "implemented", "session-wide FIFO queue with configurable global/per-provider concurrency, queue-inclusive task timeouts, individual cancellation, worktree isolation, no recursion, and no auto-merge"},
		...
		{"interface", "delegated-agent control", "implemented", "/agents inspects persisted outcomes; Session shows queued/running/approval/budget states; alt+a can cancel one child while the parent turn runs; resume marks unfinished work interrupted without restarting it"},
