...
func capabilityMatrix() []capabilityRow {
	...
	return []capabilityRow{
		...
		{"workflow", "Orchestrated Goal evidence-gated durable execution (OG-1/OG-2)", "experimental", "the model proposes a graph while the runtime owns readiness, attempts, fresh machine-observed evidence, recovery treatment, terminal state, and aggregate accounting; TUI-only explicit preview through `/orchestrate <goal>` followed by `/orchestrate approve`; status/cooperative-pause/explicit-resume/safe-retry/whole-graph-cancel expose durable state while persisted bytes remain inert; status separates proposal-plus-primary and automatic-read provider iterations, input/output tokens, honest estimated-cost availability, and elapsed time; at most two ready approved read_only nodes use governed workers before the serial primary lane advances; mutations require recognized verification against current Git state and ambiguous interrupted mutations are never replayed or manually retried; no config/headless opt-in, whole-graph aggregate enforcement or comparative benefit result, automatic writers, exact multi-worker recovery, recursion, or optional-branch/node cancellation"},
