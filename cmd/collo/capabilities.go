...
func capabilityMatrix() []capabilityRow {
	...
	return []capabilityRow{
		...
		{"tools", "read_file / list_files / search_files", "implemented", "workspace-contained, symlink-aware path guard; listing/search skip VCS metadata, dependencies, build output, caches, and virtual environments"},
		...
		{"workflow", "Orchestrated Goal evidence-gated durable execution (OG-1–OG-3A.7)", "experimental", "the model proposes a graph while the runtime owns readiness, attempts, fresh machine-observed evidence, recovery treatment, terminal state, accounting, and fixed aggregate bounds; TUI-only explicit preview through `/orchestrate <goal>` followed by `/orchestrate approve`; terminal attached or saved graphs yield to another wave in the same session through an append-only tombstone, while active graphs remain protected; proposal statuses/evidence never become runtime completion, every approved node starts pending, and `/plan off` safely cancels an unapproved proposal; status/cooperative-pause/explicit-resume/safe-retry/whole-graph-cancel expose durable state while persisted bytes remain inert; status separates proposal-plus-primary, automatic-read, and automatic-writer work under a fixed 96-iteration, 1000000-token, conditionally enforced $5 estimated-cost, and 30-minute active-wall envelope; configured max_iterations counts consecutive primary cycles without novel durable evidence rather than tool calls or productive attempt lifetime, while equivalent repeated evidence does not renew it; once an exact completion gap is recorded, a four-cycle remediation lease renews on a real workspace repair, a novel machine-observed verification failure, or evidence that closes the gate, while repeated identical failures, varied wrappers, and unrelated output do not prolong it; write-ahead effect history remains conservative, but unchanged observed repository state preserves current verification; proposal history is compacted once before node execution and later under cumulative-budget pressure while every provider summary request remains accounted; successful verification receives a state-bound node-boundary receipt, and each accepted non-final node creates a zero-provider runtime handoff that retains the durable transcript without resending the prior tool loop; rejected verification-like shell compounds receive a direct-command correction, exact redundant workspace `cd ... &&` and final `2>&1` wrappers can qualify without masking status, and graph-hidden plan/delegation tools are enforced before argument decoding; proposals prefer 1–3 substantive nodes for scoped changes and 4–6 only for broad goals, retain 12 as a hard maximum, default end-to-end changes to primary, and require the first mutating node to establish a focused test when no applicable test surface exists; at most two ready approved read_only workers remain available for substantive independent reads; isolated_write is limited to candidate-only graphs whose writers are terminal leaves, and approval preflights a clean stable Git base with exact dirty-path diagnostics; one bounded wave may run at most two pairwise-disjoint candidates through ordinary permissions, retained worktrees, scope checks, and fresh child verification; verified candidates stop blocked for review and never mutate the parent or unlock dependents; interrupted writers are not replayed; no config/headless opt-in, candidate selection/integration, exact multi-worker recovery, recursion, or optional-branch/node cancellation"},
