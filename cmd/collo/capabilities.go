...
func capabilityMatrix() []capabilityRow {
	...
	return []capabilityRow{
		...
		{"config", "first-run setup wizard", "implemented", "`collo setup` probes running local runtimes (Ollama, LM Studio, vLLM), reads each endpoint's own catalog, and configures Azure/Bedrock through a form; two real requests verify the choice before anything is written — one completion and one carrying a tool definition — and a failure is diagnosed against the endpoint, credential, or model rather than reported as a status code; re-runnable, and it never writes a secret into a configuration file; `collo setup --provider <name>` re-verifies a provider already configured, keeping its credential untouched"},
