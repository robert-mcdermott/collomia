...
func capabilityMatrix() []capabilityRow {
	...
	return []capabilityRow{
		...
		{"workflow", "Orchestrated Goal evidence-gated durable execution (OG-1/OG-2)", "experimental", "the model proposes a graph while the runtime owns readiness, attempts, fresh machine-observed evidence, recovery treatment, terminal state, accounting, and fixed aggregate bounds; TUI-only explicit preview through `/orchestrate <goal>` followed by `/orchestrate approve`; status/cooperative-pause/explicit-resume/safe-retry/whole-graph-cancel expose durable state while persisted bytes remain inert; status separates proposal-plus-primary and automatic-read work under a fixed 96-iteration, 192000-token, conditionally enforced $5 estimated-cost, and 30-minute active-wall envelope; controlled comparisons retain at most two ready approved read_only workers for substantive independent reads while primary/trivial/dependency-serial work and every parent write remain serial; mutations require recognized verification against current Git state and ambiguous interrupted mutations are never replayed or manually retried; no config/headless opt-in, automatic writers, exact multi-worker recovery, recursion, or optional-branch/node cancellation"},
