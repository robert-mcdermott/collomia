...
func capabilityMatrix() []capabilityRow {
	...
	return []capabilityRow{
		...
		{"workflow", "Orchestrated Goal evidence-gated durable execution (OG-1/OG-2)", "experimental", "the model proposes a graph while the runtime owns readiness, attempts, fresh machine-observed evidence, recovery treatment, and terminal state; TUI-only explicit preview through `/orchestrate <goal>` followed by `/orchestrate approve`; status/cancel/explicit-resume expose durable state while persisted bytes remain inert; at most two ready approved read_only nodes use governed workers before the serial primary lane advances; mutations require recognized verification against current Git state and ambiguous interrupted mutations are never replayed; no config/headless opt-in, automatic writers, exact multi-worker recovery, recursion, pause, or per-node controls"},
