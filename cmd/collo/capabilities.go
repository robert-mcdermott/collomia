...
func capabilityMatrix() []capabilityRow {
	...
	return []capabilityRow{
		...
		{"subagents", "operator steering and selective integration", "implemented", "bounded guidance is delivered only at child model boundaries; manual /agents apply is the default, while opt-in reviewed mode gives the primary agent freshness-bound inspect/apply tools backed by the same Git/base/drift, permission, atomic-write, rollback, /diff, and /undo path; no commit, merge, push, or worktree deletion"},
