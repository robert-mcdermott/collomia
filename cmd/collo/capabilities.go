...
func capabilityMatrix() []capabilityRow {
	...
	return []capabilityRow{
		...
		{"interface", "headless run + JSONL events", "implemented", "`collo run --jsonl`; embedded/published schema v1, printed by `collo schema events`; exactly one final `run.result` with stable failure/refusal/partial metadata, opaque cross-surface failure IDs, and exit codes; durable resume or session-free `--ephemeral`"},
