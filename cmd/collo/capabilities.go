...
func capabilityMatrix() []capabilityRow {
	...
	return []capabilityRow{
		...
		{"workflow", "runtime-owned primary goal graph (OG-1/OG-2A)", "experimental", "TUI-only explicit primary preview through `/orchestrate <goal>` followed by `/orchestrate approve`; proposals are read-only, fresh, pending, and acceptance-bearing; `/orchestrate status [node]`, `cancel`, and explicit `resume` expose durable node/attempt/failure/evidence state while persisted bytes remain inert; one serial primary lane retains ordinary permissions and fresh verification, with no config/headless opt-in or automatic actors"},
