...
func capabilityMatrix() []capabilityRow {
	...
	return []capabilityRow{
		...
		{"workflow", "runtime-owned primary goal graph (OG-1)", "experimental", "internal programmatic evaluation path only — no CLI/config/slash-command activation; durable schema-v1 graph snapshots and immutable attempts select dependency-ready primary work, preserve typed failures and state-bound evidence, bound retries/revisions through `propose_goal_graph_revision` and `block_goal_node`, block unverified/no-op writes, and resume without replaying ambiguous mutations; Standard mode is unchanged and automatic actors remain disabled"},
