...
- Experimental, per-session Orchestrated Goal preview: `/orchestrate <goal>` creates a read-only dependency proposal, `/orchestrate approve` starts its runtime-owned graph, and status/cancel/explicit-resume controls keep execution visible without granting new authority. Approved independent `read_only` nodes may use at most two governed automatic workers; primary work and every parent-workspace write remain serial.
