...
The Phase 6 [Orchestrated Goal strategy](docs/ORCHESTRATION_STRATEGY.md) defines Collomia's experimental **evidence-gated durable execution** mode. The model proposes a dependency graph and interprets results; the runtime owns readiness, attempts, evidence freshness, recovery treatment, terminal state, and aggregate bounds. A read node needs a successful recorded tool result tied to its claim, and a mutation needs a recognized successful verification command against current state. Graph state survives restart, but restored work remains inert until explicit resume; safe reads may be recomputed in a new attempt, while an ambiguous mutation is blocked rather than replayed. The current preview keeps one serial primary lane and can fan out at most two independently ready approved read-only nodes. End-to-end changes use the primary lane. An explicitly candidate-only graph may instead run one bounded wave of at most two pairwise-disjoint terminal isolated writers after approval verifies a clean stable commit; candidates cannot unlock dependents. Writers reuse ordinary permissions, retained Git worktrees, scope validation, and fresh child verification, and even a passing candidate stops for review without changing the parent. It can pause cooperatively at the next provider/scheduler boundary and safely retry an eligible blocked node without rewriting prior attempts. Graph status separates proposal-plus-primary, automatic-read, and automatic-writer work and shows the fixed 96-iteration, 1,000,000-token, conditionally enforced $5 estimated-cost, and 30-minute active-wall envelope. Within that hard envelope, the ordinary iteration setting is a consecutive no-progress lease for the primary lane: novel durable tool evidence renews it, equivalent repetition does not, and a process or smoke-check action cannot stale tests unless the observed repository state changes. Once the runtime records an exact completion gap, a separate four-cycle lease renews for a real workspace repair, a novel machine-observed verification failure, or evidence that closes the gate; identical failures, changing command spelling, and unrelated output cannot prolong it. If a new project has no applicable tests, proposal guidance requires its first mutating node to establish a focused smoke test instead of treating server startup or model prose as proof. Exact redundant current-workspace `cd ... &&` and final `2>&1` wrappers can qualify as verification because they preserve exit status, while masking shell composition remains rejected. Proposals prefer a small number of coherent outcome nodes instead of file-by-file steps. A newly approved graph compacts proposal history once before node execution, and later compacts proactively when the current prompt would consume at least one-eighth of the remaining graph allowance. Controlled offline comparisons retain read fan-out for substantive independent investigations while recording its extra model work; trivial and dependency-serial work remains serial. Automatic candidate selection/integration and exact multi-worker recovery remain roadmap work.
...
- Read-only planning mode with a structured, persisted plan artifact (`update_plan`, `/tasks`), plus a bounded primary-execution completion controller that requires terminal plan state and evidence, fresh verification after tracked writes, or an explicit blocked reason.
- Experimental, per-session Orchestrated Goal preview for evidence-gated durable execution: `/orchestrate <goal>` creates a read-only dependency proposal, `/orchestrate approve` starts its runtime-owned graph, and status/cooperative-pause/explicit-resume/safe-retry/whole-graph-cancel controls expose durable attempts and evidence without granting new authority. Status includes total and per-lane provider iterations, input/output tokens, estimated cost when pricing is complete, elapsed/active time, and the fixed whole-graph envelope; the proposal call is counted rather than hidden. Model claims cannot mark nodes done; approved independent `read_only` nodes may use at most two governed automatic workers, while primary work and every parent-workspace write remain serial.
...
```json
{"schema":1,"kind":"run.result","result":{"status":"ok","outcome":"done","answer":"…","session_id":"a1b2c3","changed_files":["main.go"],"duration_ms":8412},"usage":{"input_tokens":5210,"output_tokens":644}}
```

`status` remains `ok`, `error`, or `cancelled` for schema-v1 compatibility. The additive `outcome` field reports the goal-level state as `done`, `blocked`, `cancelled`, or `budget_exhausted`; optional `failure`, `partial`, and `refused` fields preserve the lower-level cause and progress. Exit codes are `0` success, `1` runtime/provider/blocked/budget failure, `2` usage or configuration failure, and `130` cancellation. Pull just the verdict with `... --jsonl | tail -1 | jq .result`.
...
Loading tolerates a torn final write (a crash mid-append) and marks any tool call with no recorded result as interrupted rather than silently replaying it. A disk error or short write is latched, shown as failed persistence in the Session tab, and makes the current TUI/headless turn visibly fail; later records are not appended behind a torn tail. The context window is managed automatically: usage-anchored estimates trigger compaction above 80% of the model's window, while an approved Orchestrated Goal also compacts at the proposal/execution boundary and when its current prompt reaches one-eighth of the remaining cumulative token allowance. The active structured plan is pinned into every request outside compactable history, and up to 16 KiB of recent failure evidence is copied verbatim into the bounded summary record; reaching that limit is marked explicitly. Compaction shortens only model context; its provider usage is still recorded and the full durable transcript remains visible after resume. `/compact [focus]` compacts on demand.
