...
- **Write-capable tasks are isolated**: `"write": true` gives that sub-agent its own `git worktree`, its own tool registry, its own permission manager, and its own audit ledger. Parallel writers can never race on the same files. Collomia never performs a Git merge, commit, or push. A worktree with real changes is retained (path and branch reported back) for `/agents apply <id>`, manual review, or the optional primary-agent reviewed-copy workflow described below; a worktree with no changes is cleaned up automatically. This requires the workspace to be a git repository.
