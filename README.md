...
`"permissions": {"sandbox": "auto"}` is the default and enables real OS enforcement when available; `"require"` fails closed instead of degrading, while `"off"` is an explicit compatibility escape hatch. `sandbox_allow_network` and `sandbox_allow_read_outside_workspace` both default to `true`, preserving package downloads, online CLIs, and broad dependency reads. Write confinement still means an external build/package cache may need a narrow `sandbox_writable_roots` grant, and the implicit minimal command environment may require an intentional `command_env: "full"` override for proxy, registry, compiler, or cloud variables. Set either `sandbox_allow_*` switch to `false` when you deliberately want that additional boundary enforced. Existing configuration files that explicitly contain `"sandbox": "off"` remain off; Collomia never silently rewrites them.
