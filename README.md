...
The Phase 6 [Orchestrated Goal strategy](docs/ORCHESTRATION_STRATEGY.md) defines Collomia's experimental **evidence-gated durable execution** mode. The model proposes a dependency graph and interprets results; the runtime owns readiness, attempts, evidence freshness, recovery treatment, and terminal state. A read node needs a successful recorded tool result tied to its claim, and a mutation needs a recognized successful verification command against the current Git workspace state before the graph can finish. Graph state survives restart, but restored work remains inert until explicit resume; safe reads may be recomputed in a new attempt, while an ambiguous mutation is blocked rather than replayed. The current preview keeps one serial primary lane and can fan out at most two independently ready approved read-only nodes. It can pause cooperatively at the next provider/scheduler boundary and safely retry an eligible blocked node without rewriting prior attempts. Graph status durably separates proposal-plus-primary and automatic-read iterations, input/output tokens, honest price availability, estimated cost, and total elapsed time. Automatic isolated writers and exact multi-worker recovery remain roadmap work.
