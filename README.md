...
- Experimental, per-session Orchestrated Goal preview: `/orchestrate <goal>` creates a read-only dependency proposal, `/orchestrate approve` starts its runtime-owned primary-only graph, and status/cancel/explicit-resume controls keep execution visible without granting new authority or enabling automatic sub-agent fan-out.
