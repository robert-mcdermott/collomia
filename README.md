...
The Phase 6 [Orchestrated Goal strategy](docs/ORCHESTRATION_STRATEGY.md) defines Collomia's experimental **evidence-gated durable execution** mode. The model proposes a dependency graph and interprets results; the runtime owns readiness, attempts, evidence freshness, recovery treatment, terminal state, and aggregate bounds. A read node needs a successful recorded tool result tied to its claim, and a mutation needs a recognized successful verification command against the current Git workspace state before the graph can finish. Graph state survives restart, but restored work remains inert until explicit resume; safe reads may be recomputed in a new attempt, while an ambiguous mutation is blocked rather than replayed. The current preview keeps one serial primary lane and can fan out at most two independently ready approved read-only nodes. It can pause cooperatively at the next provider/scheduler boundary and safely retry an eligible blocked node without rewriting prior attempts. Graph status separates proposal-plus-primary and automatic-read work and shows the fixed 96-iteration, 192,000-token, conditionally enforced $5 estimated-cost, and 30-minute active-wall envelope. Controlled offline comparisons retain fan-out for substantive independent read investigations while recording its extra model work; trivial and dependency-serial work remains serial. Automatic isolated writers and exact multi-worker recovery remain roadmap work.
